...
class TestSummarizePodcast(unittest.TestCase):
    ...
    def test_summarize_text_success(self, mock_popen):
        ...
        expected_command = ["cmd.exe", "/c", "gemini", "--model", "gemini-2.5-flash"]
        mock_popen.assert_called_once_with(expected_command, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding='utf-8')
